 ...
 * PURPOSE: Registration page — surfaces real server validation errors
 */
...
function extractError(err: any): string {
  ...
  if (d.email?.[0])              return `Email: ${d.email[0]}`;
  if (d.password?.[0])           return `Password: ${d.password[0]}`;
            ...
            {[
              ...
              <li key={item} className="flex items-center gap-2.5 text-sm text-gray-300">
                <span className="w-5 h-5 rounded-full bg-brand-500/20 border border-brand-400/30
                                 flex items-center justify-center text-brand-300 text-xs flex-shrink-0">✓</span>
                {item}
              </li>
            ))}
          </ul>
        ...
        <div className="max-w-sm w-full mx-auto lg:mx-0">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            <Input label="Full name" type="text" autoComplete="name"
              placeholder="Your full name" leftIcon={<User size={15} />}
            ...
            <Input label="Confirm password" type={showPass2 ? "text" : "password"}
              autoComplete="new-password" placeholder="••••••••"
              leftIcon={<Lock size={15} />} error={errors.password2?.message}
              rightSlot={
                <button type="button" tabIndex={-1} onClick={() => setShowPass2((s) => !s)}
                  className="text-gray-400 hover:text-gray-600 transition-colors">
