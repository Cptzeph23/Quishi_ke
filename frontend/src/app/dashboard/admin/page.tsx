 ...
 * PURPOSE: Admin dashboard — stats, property breakdowns, audit log
 */
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              <Button variant="secondary" size="sm" onClick={() => refetch()}
                leftIcon={<RefreshCw size={13} />}>Refresh</Button>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100 bg-gray-50/60">
                        {["Method","Path","Status","User","Duration","Time"].map((h) => (
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {auditLoading ? (
                        ...
                        <tr><td colSpan={6} className="py-16 text-center text-sm text-gray-400">No audit log entries found.</td></tr>
                      ) : (
                        (audit?.results ?? []).map((r: AuditLogEntry) => (
              ...
              {totalAuditPages > 1 && (
                <div className="flex items-center justify-end gap-2">
                  <Button variant="secondary" size="sm" disabled={auditPage <= 1}
                    ...
                    leftIcon={<ChevronLeft size={13} />}>Previous</Button>
                  ...
                  <Button variant="secondary" size="sm" disabled={auditPage >= totalAuditPages}
                    onClick={() => setAuditPage((p) => p + 1)}
                    rightIcon={<ChevronRight size={13} />}>Next</Button>
                </div>
              )}
            </div>
          )}
